...
async function POST(req) {
  ...
  if (!currentPassword || !newPassword) {
    return NextResponse.json({ error: "Password actual e nova password são obrigatórias." }, { status: 400 });
  }
  ...
  if (!valid) return NextResponse.json({ error: "Password actual incorreta." }, { status: 401 });
