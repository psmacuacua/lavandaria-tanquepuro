...
export default function UtilizadoresPage() {
  ...
  useEffect(() => {
    ...
    // CORRIGIDO: de "/users" para "/utilizadores"
    api.get("/utilizadores").then(d => setUsers(d.users)).catch(e => setError(e.message)).finally(() => setLoading(false));
  }, [user]);
  ...
  async function addUser() {
    ...
    try {
      // CORRIGIDO: de "/users" para "/utilizadores"
      const d = await api.post("/utilizadores", form);
  ...
  async function removeUser(id) {
    setError("");
    try {
      // CORRIGIDO: de "/users/${id}" para "/utilizadores/${id}"
      await api.del(`/utilizadores/${id}`);
