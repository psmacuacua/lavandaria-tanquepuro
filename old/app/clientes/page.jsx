...
export default function ClientesPage() {
  ...
  function load() {
    api.get("/clientes").then(d => setClients(d.clients)).finally(() => setLoading(false));
  }
  useEffect(load, []);

  async function addClient() {
    if (!form.nome.trim()) return;
    const d = await api.post("/clientes", form);
  ...
  function startEdit(c) { 
    setEditingId(c.id); 
    setEditForm({ nome: c.nome, telefone: c.telefone || "", endereco: c.endereco || "" }); 
  }

  async function saveEdit(id) {
    const d = await api.patch(`/clientes/${id}`, editForm);
  ...
  async function notify(client) {
    ...
    try {
      const d = await api.post("/notify/sms", { 
        clienteId: client.id, 
        template: "personalizada", 
        texto: `Ola ${client.nome}, contacte a Lavandaria Tanque Puro para mais informacoes sobre o seu pedido.` 
      });
